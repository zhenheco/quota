# Slice 08 — 去振禾化 + 品牌 setup + seed-brand CLI — Implementation Plan

> For agentic workers: TDD where feasible，frequent commits。STAY on branch `feat/quote-system`；不新建分支；不 cd 他專案。

**Goal:** 讓開源自架者零振禾殘留地換成自己品牌：出廠全空白、振禾素材移為 demo、首次 setup 引導、`seed-brand` CLI（HTTP）一次灌品牌、README onboarding。

**Architecture:** 新增 token-gated brand-upload endpoint（複用 `brand.ts` putBrandAsset + `auth` + companyRepo），`seed-brand` 為 HTTP client（像 MCP，吃 URL+token），與「API 為唯一真實來源」一致、可對任何部署用。CSS 金色主題為 app 預設美學（自架者可改 CSS）；本 slice 的「去振禾」針對**公司資料**（名/地址/匯款/logo/章），非主題色。
...
- Move：`git mv seed/brand examples/demo-brand`；`git mv reference examples/demo-reference`（振禾原稿 xlsx 當 demo 參考）。
...
- 確認**無振禾公司資料硬編**於 src（grep 「振禾/玉山/範例客戶」與具體地址/帳號）；company 一律來自 D1（slice 02 已種空白）。若發現任何硬編值 → 移除/改空白。
...
- demo 素材移到 `examples/demo-brand/`，README 註明僅供示範、非預設。1.4M 振禾 xlsx 已在 git 歷史（移位即可，不重寫歷史）。

## Tasks

- [ ] **Task 1**：去振禾掃描 + 移檔。`git mv` seed/brand→examples/demo-brand、reference→examples/demo-reference。grep 確認 src 無振禾硬編（有則清）。更新任何引用路徑（如測試讀 seed/brand 的，改 examples/demo-brand）。驗 `pnpm test`+`pnpm build` 仍綠。Commit：`refactor: relocate 振禾 brand/reference to examples as demo data (slice-08)`。
...
- [ ] 出廠無振禾硬編值；`seed/brand/*` 與 `reference/*.xlsx` 移到 `examples/`。
