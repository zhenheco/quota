...
- **Acceptance criteria**:
  - [ ] 出廠 `company_profile` 全空白，無振禾硬編值；振禾 `seed/brand/*` + `reference/*.xlsx` 移到 `examples/demo-brand/` 標 demo。
...
- `reference/20260519_範例客戶_行銷_報價單.xlsx`（1.4M，振禾原稿）目前在 git 內 → Slice 7 移到 `examples/`，評估 gitignore/LFS。
