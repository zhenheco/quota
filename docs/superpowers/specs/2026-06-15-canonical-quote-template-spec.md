...
老闆（振禾）用 Quota 開報價單給客戶。同一張報價單，**網站上看到的樣子、編輯時右邊預覽、跟下載的 PDF 長得不一樣**——系統內有兩套版型，看起來不一致、不專業。老闆已挑定要的版型（「報 價 單」精緻金棕 editorial 版，Design C），希望**以後網站生成的每一張報價單（線上預覽 + PDF）永遠長這樣**。另外賣方抬頭目前無法填「聯絡人」，做政府計畫附件時資訊不完整。
...
1. As 振禾老闆, I want 線上 `/q/[id]` 報價單頁顯示 Design C 版型, so that 寄連結給客戶時看起來專業一致。
2. As 振禾老闆, I want 下載的 PDF 與線上頁**像素一致**的 Design C, so that 不會出現「網頁一套、PDF 另一套」。
3. As 振禾老闆, I want 編輯報價單時右邊即時預覽就是 Design C, so that 所見即所得、不必下載才知道長相。
4. As 振禾老闆, I want 在 `/settings` 填「賣方聯絡人」, so that 抬頭能顯示 範例負責人 作為聯絡人。
5. As 振禾老闆, I want 賣方抬頭顯示 名稱／統編／地址／聯絡人／電話, so that 政府計畫附件資訊完整。
6. As 振禾老闆, I want 稅率 > 0 時顯示 未稅小計／營業稅 N%／總計（含稅）三列、稅率 0 時隱藏, so that 含稅與未稅報價都正確。
7. As 振禾老闆, I want 品項顯示 品名 + 多行說明（自由文字、保留換行）, so that 9 點服務內容完整呈現。
8. As 振禾老闆, I want **新建（未存檔）** 報價單時右邊預覽也能顯示 logo／印章／存摺圖, so that 建立過程就看到完整版型（不必先存檔）。
9. As 振禾老闆, I want 編輯品項（增删、改數量/單價）時預覽即時更新且版型不破, so that 即時預覽可靠。
10. As 振禾老闆, I want 客戶統編為空時該列自動隱藏、有值才顯示, so that 沒有空白欄。
...
13. As 振禾老闆, I want 部署前先備份 prod D1、部署後實機驗證, so that 不會弄壞線上真實資料。
14. As 振禾老闆, I want xlsx 下載維持原樣不動, so that 既有 Excel 流程不受影響。
